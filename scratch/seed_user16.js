...
console.log('Incidencias cols:', incSchema.map((c: any) => c.name).join(', '));
...
// Also copy plan from user 1 PlanesDiarios
const planesSchema = db.prepare('PRAGMA table_info(PlanesDiarios)').all();
console.log('PlanesDiarios cols:', planesSchema.map((c: any) => c.name).join(', '));

const planes1 = db.prepare('SELECT * FROM PlanesDiarios WHERE user_id = 1 OR user_id IS NULL').all();
console.log('PlanesDiarios user 1/null:', planes1.length);
